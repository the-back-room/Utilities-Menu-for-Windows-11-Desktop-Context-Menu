...
5. Navigate to the Addons directory and double-click the registry file to add or remove your desired addons.

To remove the Utilities Menu from your desktop context menu, double-click the `Remove Utilities Menu.reg` file.

## Addon Instructions
The following instructions are for addons that require additional steps to install. If a desired addon is not listed here; then it does not require any additonal steps and can be installed by simply using the registry file provided in the Addons directory.
...
- **Enable Modern Apps**: Double-click the `Enable Modern Apps.reg` file included in the `explorer7` directory to enable support for modern apps in explorer7.
...
1. Download UWPSpy from [ramensoftware.com/uwpspy](https://ramensoftware.com/uwpspy) and extract it's contents into `C:\Utilities\UWPSpy`
